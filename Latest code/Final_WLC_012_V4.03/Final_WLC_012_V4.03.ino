 ...
 *  Date : 23/09/2024
 *  V4.03 is a common code of WLC & WLS.  
 *  This code is for 1600 watt ring heaters only. 
 *  This code is using All in one PCB V1.3 only.
 *  This code validates the hardware's new connector, PCB V1.4.
 */
...
//  Serial2.println("siddhesh");
  ...
  {  
    ...
    led.DISPLAY_DIGIT1(24, 4, 0, 3) ;      //----------------------4.03 
    DOT1_1=1;                              //----------------------dot on                       
  }
  else 
  {
    ...
    else
    {
      switch (SELECTION_PARAMETER)
      {
        ...
        case Slow_Fast_Heating :
          ...
          EEPROM.write(132, select_heating_type);                            
          break;
        ...
        case Cal1_Point :
          if (!DEGREE_VALUE_SET)
          {
            ...
            ERROR_CAL = ((( CAL1_VALUE_SAVED_DEG1 + (  CAL1_VALUE_SAVED_DEG2 / 10.0)) * 10) - TEMPERARY_SAVED_VARIABLE_FOR_CAL1_DEG) / 10; 
             ERROR_SET_IN_DEG_FLAG=1;  ERROR_SET_IN_FAH_FLAG=0;
          }
          else
          {
            CAL1_VALUE_SAVED_FAH = SET_LONG_VALUE[Cal1_In_Fah];  

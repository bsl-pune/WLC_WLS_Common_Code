...
void Pclass :: SET_SHORT_PRESS_FUNCTION()   //------------------function for main process execution
{
  switch (ENTER_BUT)
  {
    ...
    case 2 :
          ...
          { 
            digitalWrite(SET_PARA_LED, LOW);                                             //Fahrenheit start procedure green LED glow
...
//          else     
          ...
          while(SET_MIN >0) 
          {
            Procss.toggle_ticker_update();
            if(((TEMP_DEG > SET_TEMP_DEG + 10) && DEGREE_VALUE_SET==0) || ((TEMP_FAH > SET_TEMP_FAH + 10) && DEGREE_VALUE_SET==1))   //if the temparature increases by 10 degrees than set temperature, it will show SSR fail error.
            {
              i=0;
              checkTempRaised=1;
              digitalWrite(safteyRelay, LOW);                                                                                        //if temp raises beyond set temperature, Contactor will get OFF, heater circuit will be open.
              ENTER_BUT=0;             
              SET_MIN=0;
            }
            else
            {
              checkTempRaised=0;
               ...
               if(STIR_VALUE_SET && select_product_type)  digitalWrite(PUMP, HIGH); 
               ...
               else
               {
                 if(!DEGREE_VALUE_SET)  
                 {
                   ...
                   {     
                     ...
                     else if (TEMP_DEG < PID_SETPOINT_To_Show && soak_once_flag==0) 
                   {
                     COUNTDOWN_START=0; 
                   }
                 }
                 ...
                 {
                  if (TEMP_FAH >= PID_SETPOINT_To_Show)
                  {
                    COUNTDOWN_START=1; 
                    soak_once_flag=1;
                  }
                  else if (TEMP_FAH < PID_SETPOINT_To_Show  && soak_once_flag==0)
                  {
                   COUNTDOWN_START=0;   
                  }
               ...
               if(((TEMP_DEG >= PID_SETPOINT_To_Show) && DEGREE_VALUE_SET==0 ) || ((TEMP_FAH >= PID_SETPOINT_To_Show) && DEGREE_VALUE_SET==1 ))  //Fahrenheit & Celsius set point reached buzer indication 
               {                                       
                 ...
                 if(PUMP_FLAG && select_product_type)
                  {
                    ...
                    ALL_LED_OFF();    
                    digitalWrite(TIMER_LED1, LOW);      
                    led.DISPLAY_DIGIT2(12, 16, 25, 17) ; //Stir
                    if(!SET_VALUE[5])     led.DISPLAY_DIGIT1(0, 21, 21, 24); //OFF
...
//                i++;
       ...
       case Cal2HighPointTemp_Enter_Maintain :                                                            //Inprocess-Entering value High
        ...
        case Slow_Fast_Heating: 
             ...
             DOT2_1=0; 
             ALL_LED_OFF();
             led.DISPLAY_DIGIT2(30, 15, 13, 16) ; //HEAT     
             if(i<VALUE_FOR_BLINK)  
             {
                if(!BLINK_STATS)  led.DISPLAY_DIGIT1(24, 24, 24, 24) ;
                else i=VALUE_FOR_BLINK;  
             ...
             { 
                 if(!SET_LONG_VALUE[SELECTION_PARAMETER])  led.DISPLAY_DIGIT1(12, 26, 23, 36); //SLOW
                 else                                      led.DISPLAY_DIGIT1(21, 13, 12, 16); //FAST
             }
             if(i>TOTAL_VALUE_FOR_BLINK)    i=0;  
             break;
  ...
  { 
     ...
     case Cal2HighPointTemp_Enter_Maintain : // maintain temp till user interupt for entering temp
      ...
      case Cal2_Complete_Display :
       ...
       case Slow_Fast_Heating:  //New feature added
             DOT1_0=0;
             DOT1_1=0;
             DOT2_1=0; 
             ALL_LED_OFF();
             SET_LONG_VALUE[SELECTION_PARAMETER]= select_heating_type;
             led.DISPLAY_DIGIT2(30, 15, 13, 16) ;  //HEAT
             led.DISPLAY_DIGIT1(12, 15, 26, 16) ;  //SELT
             break;

 ...
 *  Date : 01/07/2024.
 *  V4.01 is a common code of WLC & WLS.  
 *  Soak time problem solve.
 *  This code is for ring heaters only. 
 *  Heater capacity 700 W (350 X 2) Two Ring Heater Used.
...
//  Serial2.println("siddhesh");
  ...
  {  
    ...
    led.DISPLAY_DIGIT1(24, 4, 0, 1) ;      //----------------------4.01 
    DOT1_1=1;                              //----------------------dot on                       
  }
  else 
  {
      VersionDisplay.stop(); 
      if(TEMP_DEG_RAW >=120)     Probe_Error();                 //-----------------------Temp error detection loop(Without Probe PCB Working)113
      else
      { 
         but_check();

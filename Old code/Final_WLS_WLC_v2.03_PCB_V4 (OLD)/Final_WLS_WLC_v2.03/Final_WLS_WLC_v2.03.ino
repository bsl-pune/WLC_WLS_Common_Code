 ...
 *  Date : 26/08/2023
 *  v2.03 is a common code of WLC & WLS.
 *  At service menu Product Type Selection menu is provided in which we can select product type WLC/WLS.
 *  Added one more point in the service menu is process run time log as same as we provided in WBP.
 *  This code is for mica heaters only. 
 *  This code is for old display PCB V4 only.
 */
...
//  Serial1.begin(9600);
  ...
  button3.attachLongPressStop(INC_LONGPRESS_STOP);
...
//  Serial1.println("1");
  ...
  {  
    ...
    led.DISPLAY_DIGIT1(24, 2, 0, 3) ;      //----------------------2.03 //Siddhesh
    DOT1_1=1;                              //----------------------dot on                       
  }
  else 
  {
      VersionDisplay.stop(); 
      if(TEMP_DEG_RAW >=113)     Probe_Error();                 //-----------------------Temp error detection loop(Without Probe PCB Working)
      else
      { 
         but_check();
...
//      lock_set();//------------Go to lock settings mode.
     Tick.BUZZ_INTERVAL(500);  
      if (!ENTER_BUT && !LONG_PRESS )     
      {
          ...
          RST_BUTT = 1;
          Service_Menu=1;
          SELECTION_PARAMETER = Product_Selection_Parameter;  //Calibration_Selection;
      }
    }
  }
  else if (but1 == 1)
